export function assembleEpisode01Consequences(
  ...
  const eveningGate = { kind: 'event_completed', event_id: 'e01_08n_instruction_return', minimum_count: 1 };
  return baseEvents.flatMap(event => {
    ...
    return event.event_id === 'e01_08_reactions'
      ? [next, ...consequenceEvents, ...inspectionEvents, ...responsibilityEvents, ...tbmGapEvents, ...restartEvents, ...stopworkEvents, ...instructionEvents]
      : [next];
  });
}
