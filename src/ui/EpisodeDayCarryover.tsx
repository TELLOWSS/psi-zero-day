...
import { episode01DayCarryover } from '../app/episode01-day-carryover';

export function EpisodeDayCarryover({eventId,flags,t}:{
  readonly eventId:string|null|undefined;
  readonly flags:FlagMap|undefined;
  readonly t:(id:string)=>string;
}){
  const model=episode01DayCarryover(eventId,flags);
  if(!model) return null;
  return <section className="episode-day-carryover" data-phase={model.phase} data-doctrine={model.doctrine_id} aria-label={t(model.title_text_id)}>
    <header><span>{t('ui.field_reality_doctrine.badge')} · {t(model.eyebrow_text_id)}</span><strong>{t(model.title_text_id)}</strong></header>
    <ol className="day-carryover-timeline">
      {model.stages.map(stage=><li key={stage.time} data-state={stage.state}><time>{stage.time}</time><span>{t(stage.label_text_id)}</span></li>)}
    </ol>
    <div className="day-carryover-cards">
