...
Status: CONTRACT COMPLETE / FINAL ART 0 OF 9
...
| suspended_load | `assets/episode01/scene-elements/suspended-load.webp` | 768×768 | 136 px | 0.50, 0.62 |
...
All nine slots require WebP alpha transparency. No text, labels, warnings, site names, logos, floor plate, background scenery, or UI may be baked into the cutout.
...
This validates slot count, unique IDs/paths, WebP destination paths, minimum dimensions, normalized pivots, map scale, and required alpha metadata. It does not require final binary files yet.

When all final scene-element art is ready, run the strict binary gate:

`npm run assets:production-scene-elements-check`

The strict gate requires all 9 files, valid WebP headers, each per-slot minimum dimension, and actual WebP alpha transparency.
...
The original Episode 01 core production gate remains exactly 17 files (1 background + 16 character images). Scene-element production is a separate 9-slot gate and does not distort the core release count.
